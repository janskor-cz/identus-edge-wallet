...
export const Chat: React.FC<ChatProps> = ({ messages, connection, onSendMessage }) => {
  ...
  const formatTime = (message: SDK.Domain.Message): string => {
    if (!message.createdTime) {
      console.log('⏰ [Chat] No createdTime for message:', message.id);
      return 'No time';
    }

    const date = new Date(message.createdTime);
    console.log('⏰ [Chat] Formatting time:', {
  ...
  const isOwnMessage = (message: SDK.Domain.Message): boolean => {
    ...
    console.log('🔍 [Chat] Message direction check:', {
      messageId: message.id?.substring(0, 8) + '...',
      messageFrom: messageFromDID?.substring(0, 50) + '...',
      myDID: myDID.substring(0, 50) + '...',
      isOwnMessage: messageFromDID === myDID
    });
