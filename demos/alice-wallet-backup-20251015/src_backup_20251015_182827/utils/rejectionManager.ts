...
export class MessageRejectionManager {
  ...
  async initialize(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        console.log('✅ [REJECTION MANAGER] IndexedDB initialized successfully');
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('rejections')) {
          ...
          console.log('🏗️ [REJECTION MANAGER] Created rejections object store');
        }
      };
    });
  }
  ...
  async rejectMessage(
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log(`✅ [REJECTION MANAGER] Rejected message: ${message.id} from ${rejectionRecord.fromDID.substring(0, 30)}...`);
        resolve();
      };
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (rejectionRecord.expiresAt <= now) {
          console.log(`🧹 [REJECTION MANAGER] Rejection expired for message: ${messageId}`);
        ...
        console.log(`🚫 [REJECTION MANAGER] Message ${messageId} is rejected (expires: ${new Date(rejectionRecord.expiresAt).toLocaleString()})`);
        resolve(true);
      };
  ...
  async isDIDBlocked(fromDID: string): Promise<boolean> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (shouldBlock) {
          console.log(`🚫 [REJECTION MANAGER] DID ${fromDID.substring(0, 30)}... is temporarily blocked (${recentRejections.length} recent rejections)`);
        }
  ...
  async removeRejection(messageId: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log(`🗑️ [REJECTION MANAGER] Removed rejection: ${messageId}`);
        resolve();
      };
  ...
  async cleanupExpiredRejections(): Promise<number> {
    ...
    if (cleanedCount > 0) {
      console.log(`🧹 [REJECTION MANAGER] Cleaned up ${cleanedCount} expired rejections`);
    }
  ...
  async clearAllRejections(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log('🧹 [REJECTION MANAGER] Cleared all rejection records');
        resolve();
      };
