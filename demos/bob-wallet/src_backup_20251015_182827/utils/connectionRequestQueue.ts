...
export class ConnectionRequestQueueManager {
  ...
  async initialize(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        console.log('✅ [CONNECTION QUEUE] IndexedDB initialized successfully');
        resolve();
      };
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('connection_requests')) {
          ...
          console.log('🏗️ [CONNECTION QUEUE] Created connection_requests object store');
        }
      };
    });
  }
  ...
  async addConnectionRequest(
    ...
    console.log(`🔍 [CONNECTION QUEUE] Checking for duplicate message.id: ${message.id}`);
    const existingRequest = await this.findRequestByMessageId(message.id);

    if (existingRequest) {
      console.log(`⚠️ [CONNECTION QUEUE] Request with message.id ${message.id} already exists. Skipping duplicate. Existing request ID: ${existingRequest.id}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log(`✅ [CONNECTION QUEUE] Added NEW request: ${requestId} for message.id: ${message.id}`);
        resolve(requestId);
      };
  ...
  async getPendingRequests(): Promise<ConnectionRequestItem[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        console.log(`📋 [CONNECTION QUEUE] Retrieved ${validRequests.length} pending requests`);
    ...
    return new Promise((resolve, reject) => {
      ...
      getRequest.onsuccess = () => {
        ...
        putRequest.onsuccess = () => {
          console.log(`✅ [CONNECTION QUEUE] Updated request ${requestId} status to ${status}`);
          resolve();
        };
  ...
  async removeRequest(requestId: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log(`🗑️ [CONNECTION QUEUE] Removed request: ${requestId}`);
        resolve();
      };
  ...
  async cleanupExpiredRequests(): Promise<number> {
    ...
    console.log(`🧹 [CONNECTION QUEUE] Cleaned up ${cleanedCount} expired requests`);
    return cleanedCount;
  }
  ...
  async clearAllRequests(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log('🧹 [CONNECTION QUEUE] Cleared all requests');
        resolve();
      };
  ...
  async deduplicateRequests(): Promise<number> {
    ...
    for (const [messageId, requests] of requestsByMessageId.entries()) {
      if (requests.length > 1) {
        ...
        console.log(`🔍 [DEDUPLICATION] Found ${requests.length} duplicates for message.id: ${messageId}`);
        console.log(`  ✅ Keeping earliest: ${toKeep.id} (timestamp: ${new Date(toKeep.timestamp).toISOString()})`);

        for (const duplicate of toRemove) {
          console.log(`  🗑️ Removing duplicate: ${duplicate.id} (timestamp: ${new Date(duplicate.timestamp).toISOString()})`);
    ...
    console.log(`🧹 [DEDUPLICATION] Removed ${removedCount} duplicate requests`);
    return removedCount;
  }
}
