...
const WasmMemoryGuard: React.FC = () => {
  ...
  const cleanupWasmMemory = () => {
    console.log('🧹 [WasmGuard] Performing aggressive WASM memory cleanup...');

    try {
      // Force garbage collection if available
      if (typeof window !== 'undefined' && (window as any).gc) {
        (window as any).gc();
        console.log('✅ [WasmGuard] Manual GC triggered');
      }
          ...
          try {
            ...
            for (const db of dbs) {
              ...
              if (db.name && (db.name.includes('cache') || db.name.includes('temp'))) {
                console.log(`🗑️ [WasmGuard] Clearing cache DB: ${db.name}`);
      ...
      console.log('✅ [WasmGuard] WASM memory cleanup completed');
    } catch (error) {
  ...
  useEffect(() => {
    console.log('🛡️ [WasmGuard] WebAssembly Memory Guard initialized');
    ...
    const handleRouteChange = () => {
      console.log('🔄 [WasmGuard] Route change detected, cleaning up...');
      cleanupWasmMemory();
    };
    ...
    if (typeof window !== 'undefined') {
      ...
      document.addEventListener('visibilitychange', () => {
        if (!document.hidden) {
          console.log('👁️ [WasmGuard] Page visible, checking memory...');
          checkMemoryPressure();
        }
      });
    }
    ...
    return () => {
      ...
      console.log('🛡️ [WasmGuard] WebAssembly Memory Guard unmounted');
    };
  }, []);
