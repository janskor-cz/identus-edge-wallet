...
export function storeConfiguration(config: WalletConfiguration): boolean {
  try {
    console.log('[ConfigStorage] Storing configuration:', config.credentialId);
    ...
    if (existingIndex >= 0) {
      ...
      console.log('[ConfigStorage] Updated existing configuration');
    } else {
      // Add new configuration
      allConfigs.push(storedConfig);
      console.log('[ConfigStorage] Added new configuration');
    }
    ...
    console.log('[ConfigStorage] ✅ Configuration stored successfully');
    return true;
...
export function getActiveConfiguration(): WalletConfiguration | null {
  try {
    ...
    if (!stored) {
      console.log('[ConfigStorage] No active configuration found');
      return null;
    }
...
export function setActiveConfiguration(
  ...
  try {
    console.log('[ConfigStorage] Setting active configuration:', config.credentialId);
    ...
    console.log('[ConfigStorage] Encrypting and storing API key...');
    const apiKeyStored = storeEncryptedApiKey(
      config.enterpriseAgentApiKey,
      config.enterpriseAgentWalletId,
      {
        enterpriseAgentUrl: config.enterpriseAgentUrl,
        walletId: config.enterpriseAgentWalletId,
    ...
    if (!apiKeyStored) {
      console.warn('[ConfigStorage] ⚠️ API key encryption failed, but configuration still applied');
      // Don't fail the entire operation if API key storage fails
      // User can manually re-apply configuration if needed
    } else {
      console.log('[ConfigStorage] ✅ API key encrypted and stored successfully');
    }
    ...
    console.log('[ConfigStorage] ✅ Active configuration set successfully');
...
export function clearActiveConfiguration(): boolean {
  try {
    console.log('[ConfigStorage] Clearing active configuration');

    const activeConfig = getActiveConfiguration();

    if (activeConfig) {
      // Clear encrypted API key
      console.log('[ConfigStorage] Clearing encrypted API key...');
      const apiKeyCleared = clearApiKey(activeConfig.enterpriseAgentWalletId);
      if (apiKeyCleared) {
        console.log('[ConfigStorage] ✅ API key cleared successfully');
      } else {
        console.warn('[ConfigStorage] ⚠️ API key cleanup failed or no key found');
      }
    ...
    console.log('[ConfigStorage] ✅ Active configuration cleared');
    return true;
...
export function removeConfiguration(credentialId: string): boolean {
  try {
    console.log('[ConfigStorage] Removing configuration:', credentialId);
    ...
    // Clear encrypted API key for this configuration
    console.log('[ConfigStorage] Clearing encrypted API key for removed configuration...');
    const apiKeyCleared = clearApiKey(configToRemove.config.enterpriseAgentWalletId);
    if (apiKeyCleared) {
      console.log('[ConfigStorage] ✅ API key cleared successfully');
    } else {
      console.warn('[ConfigStorage] ⚠️ API key cleanup failed or no key found');
    ...
    console.log('[ConfigStorage] ✅ Configuration removed successfully');
    return true;
...
export function clearAllConfigurations(): boolean {
  try {
    console.log('[ConfigStorage] Clearing all configurations');
    ...
    console.log('[ConfigStorage] ✅ All configurations cleared');
    return true;
