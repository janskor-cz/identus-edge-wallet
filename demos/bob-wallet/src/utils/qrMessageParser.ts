...
function parseOOBInvitation(oobBase64: string, originalUrl: string): ScanResult {
  ...
  try {
    ...
    // Check for CA identity verification goal
    const isCAVerification =
      invitation.body?.goal_code === 'ca-identity-verification' ||
      invitation.body?.goal?.includes('Certification Authority');

    const messageType: MessageType = isCAVerification
      ? 'ca-identity-verification'
      : 'oob-invitation';
...
function detectMessageTypeFromJSON(data: any, rawData: string): ScanResult {
  ...
  if (
    ...
    const isCAVerification =
      data.body?.goal_code === 'ca-identity-verification' ||
      data.body?.goal?.includes('Certification Authority');

    return {
      rawData,
      messageType: isCAVerification ? 'ca-identity-verification' : 'oob-invitation',
      parsedData: data,
      routeAction: {
        page: '/oob',
        queryParams: { invitation: rawData }
