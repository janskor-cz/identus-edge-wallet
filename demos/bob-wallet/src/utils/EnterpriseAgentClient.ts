 ...
 *
 * Security:
 * - API keys retrieved from encrypted storage
...
export class EnterpriseAgentClient {
  ...
  constructor(config: WalletConfiguration) {
    ...
    if (!this.apiKey) {
      console.warn('[EnterpriseAgentClient] No API key found for wallet:', this.walletId);
      console.warn('[EnterpriseAgentClient] Client will not be able to authenticate requests');
    } else {
      console.log('[EnterpriseAgentClient] Client initialized for:', this.baseUrl);
    }
  }
  ...
  private async request<T = any>(
    ...
    try {
      ...
      console.log('[EnterpriseAgentClient] Request:', options.method || 'GET', url);
      ...
      console.log('[EnterpriseAgentClient] ✅ Request successful');
      return {
        success: true,
        data,
        status: response.status
  ...
  public async acceptInvitation(invitationUrl: string, label?: string): Promise<ApiResponse<ConnectionRecord>> {
    ...
    try {
      ...
      if (oobParam) {
        oobPayload = oobParam;
        console.log('[EnterpriseAgentClient] Extracted _oob parameter from URL');
      }
    } catch (error) {
      // If not a valid URL, assume it's already just the OOB payload
      console.log('[EnterpriseAgentClient] Using invitation as-is (not a URL)');
    }
  ...
  /**
   * Accept credential request from employee
   *
   * @param recordId - Credential record ID
