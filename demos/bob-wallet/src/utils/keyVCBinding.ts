...
export async function getVCX25519PublicKey(vc: any): Promise<string | undefined> {
  ...
  // v3.0.0 Security Clearance VCs always have native X25519 keys
  console.error('❌ [getVCX25519PublicKey] No X25519 key found - v3.0.0 VC required');
  return undefined;
}
 ...
 *
 * @example
 * const dualKey = await generateDualSecurityKey("Bob's Clearance Key");
 ...
 *
 * @example
 * const dualKey = await generateDualSecurityKey("Bob's Key");
