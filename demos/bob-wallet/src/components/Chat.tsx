...
import { getVCClearanceLevel, validateSecurityClearanceVC, getSecurityKeyByFingerprint } from '../utils/keyVCBinding';
        ...
        try {
          ...
          const userVC: any = credentials.find((cred: any) => {
            try {
              ...
              // Check if we have the private key for this VC (confirms it's OUR VC)
              const hasPrivateKey = getSecurityKeyByFingerprint(x25519Fingerprint) !== undefined;
              if (!hasPrivateKey) {
                console.log('⏭️ [Chat] Skipping VC - no private key (belongs to someone else)');
                return false;
              }

              // v3.0.0 Security Clearance VC found
              return true;
            } catch (e) {
              return false;
            }
          ...
          // Get dual-key from storage by X25519 fingerprint
          const userKey = getSecurityKeyByFingerprint(userX25519Fingerprint);
          if (!userKey) {
            console.error('❌ [Chat] X25519 private key not found');
            newDecrypted.set(message.id, '🔒 [X25519 key not found in storage]');
            hasNewDecryptions = true;
            continue;
          }
  ...
  return (
    <div className="flex flex-col h-full max-h-[600px] bg-white dark:bg-gray-800 rounded-lg shadow-lg">
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 p-4">
        {/* ✅ NEW: Security Clearance Revocation Warning */}
        {clearanceRevoked && (
