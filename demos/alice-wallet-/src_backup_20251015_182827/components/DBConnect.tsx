...
export function DBConnect({ children }) {
    ...
    const { db, dispatch, connectDatabase } = useMountedApp();
    ...
    async function onConnectClick() {
        ...
        try {
            // connectDatabase is already bound by bindActionCreators, so call it directly
            const result = await connectDatabase({
                encryptionKey: Buffer.from(password)
            });

            if (result.type !== 'connectDatabase/fulfilled') {
                console.error('❌ [DBConnect] Database connection failed:', result);
            }
        } catch (error) {
            console.error('❌ [DBConnect] Database connection error:', error);
        }
    }
