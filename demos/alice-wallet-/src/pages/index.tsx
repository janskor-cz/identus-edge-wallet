...
const Agent: React.FC<{}> = props => {
  ...
  console.log('🚀 [index.tsx] Agent component mounting/rendering');
  ...
  console.log('🔍 [index.tsx] Component state:', {
  ...
  // Separate useEffect for starting agent - runs only once when agent becomes available
  useEffect(() => {
    if (app.agent.instance && !app.agent.hasStarted && !app.agent.isStarting) {
      console.log('🚀 [index.tsx] Agent initialized, starting agent automatically...');
      console.log('🔍 [index.tsx] Agent state check:', {
  ...
  }, [app.agent.instance]); // Only depends on agent.instance existence, not the full app.agent object
