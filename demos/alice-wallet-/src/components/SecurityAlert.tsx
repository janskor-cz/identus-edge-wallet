...
export const SecurityAlert: React.FC<SecurityAlertProps> = ({
  ...
  return (
    <div className={`security-alert ${config.bgColor} ${config.borderColor} border rounded-lg p-4 ${className}`}>
      {/* Header */}
        ...
        <div>
          ...
          <div className="flex items-center space-x-2 mt-1">
            ...
            <span className={`px-2 py-1 rounded text-xs font-bold ${
              getRiskLevel() === 'HIGH' ? 'bg-red-200 text-red-800' :
              getRiskLevel() === 'MEDIUM' ? 'bg-yellow-200 text-yellow-800' :
      ...
      {/* Recommendation */}
      <div className={`mb-4 p-3 bg-white bg-opacity-50 rounded border ${config.textColor}`}>
      ...
      <div className="flex space-x-3">
        ...
        <button
          ...
          className={`flex-1 px-4 py-2 ${config.buttonColor} text-white font-medium rounded transition-colors`}
        >
