...
import { verifyKeyVCBinding, getVCClearanceLevel, getSecurityKeyByFingerprint, validateSecurityClearanceVC } from '../utils/keyVCBinding';
    ...
    if (presentationRequests.length) {
        ...
        for (const requestMessage of presentationRequests) {
            try {
                ...
                // Extract schema ID from message body if available
                // For Security Clearance VCs, use the schema ID "SecurityClearanceLevel v3.0.0"
                let schemaId: string | undefined;
                try {
                    const bodyData = requestMessage.body;
                    if (bodyData && typeof bodyData === 'object') {
                        // Check for schema field in message body
                        schemaId = (bodyData as any).schemaId || (bodyData as any).credentialSchema;
                    }
                    // Fallback: If no schema in body, use Security Clearance schema for VC handshake
                    if (!schemaId) {
                        schemaId = 'SecurityClearanceLevel v3.0.0';
                    }
                } catch (err) {
                    console.warn('⚠️ [VC-HANDSHAKE] Could not extract schema ID, using default');
                    schemaId = 'SecurityClearanceLevel v3.0.0';
                }
                ...
                // AUTO-SEND: Automatically send matching credential if found
                try {
        ...
        const checkInterval = setInterval(async () => {
            try {
                ...
                if (Date.now() - startTime > timeoutMs) {
                    ...
                    reject(new Error(
                        'VC handshake timeout: Recipient did not respond with their Security Clearance VC. ' +
                        'They may be offline or have not accepted the request.'
                    ));
                    return;
                }
                ...
                const presentationResponse = allMessages.find(msg => {
                    ...
                    } else if (msg.createdTime instanceof Date) {
                        msgTime = msg.createdTime.getTime();
                    } else {
                        msgTime = 0;
                    }
                ...
                if (presentationResponse) {
                    ...
                    if (recipientVC) {
                        ...
                        console.log('✅ [VC-HANDSHAKE] FINAL ACCEPTED VC DETAILS:', {
                            schema: recipientVC.credentialSchema?.[0]?.id?.substring(0, 80),
                            subjectDID: (recipientVC.credentialSubject?.id || recipientVC.subject?.id)?.substring(0, 60),
                            holderName: recipientVC.credentialSubject?.holderName || recipientVC.claims?.[0]?.holderName,
                            clearanceLevel: recipientVC.credentialSubject?.clearanceLevel || recipientVC.claims?.[0]?.clearanceLevel
                        });
        ...
        // NEW MODE: Content + security level (encryption support)
            ...
            if (securityLevel > SecurityLevel.UNCLASSIFIED) {
                ...
                for (const oldVC of oldRecipientVCs) {
                    ...
                    await agent.pluto.deleteCredential(oldVC.id);
                }
                ...
                const oldPresentations = allMessages.filter(msg => {
                    ...
                    const msgTime = typeof msg.createdTime === 'number'
                        ? msg.createdTime
                        : msg.createdTime instanceof Date
                            ? msg.createdTime.getTime()
                            : 0;
                ...
                if (!recipientVC) {
                    ...
                    try {
                        ...
                        throw new Error(
                            `Cannot send classified message: VC handshake failed. ` +
                            `${handshakeError.message || 'Recipient did not respond with Security Clearance VC.'}`
                        );
                    }
                }
                ...
                const senderKey = getSecurityKeyByFingerprint(senderX25519Fingerprint);
                if (!senderKey) {
                    throw new Error('X25519 encryption key not found in storage. Generate a new v3.0.0 Security Clearance VC.');
                }
                ...
                const recipientSubject = recipientVC.credentialSubject || recipientVC.subject;
