...
export const ConnectionRequest: React.FC<ConnectionRequestProps> = ({
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        for (let i = 0; i < localStorage.length; i++) {
          ...
          if (key && key.startsWith('invitation-')) {
            try {
  ...
  // Extract and parse attached credential presentations
  const extractAttachedCredential = () => {
    try {
      ...
      // Check message body for embedded credentials
      const body = typeof message.body === 'string' ? JSON.parse(message.body) : message.body;
  ...
  useEffect(() => {
    const loadCredentialData = async () => {
      ...
      // First try to find credential attached directly to connection request
