...
import { getItem, removeItem, getKeysByPattern } from '@/utils/prefixedStorage';
import { saveConnectionMetadata } from '@/utils/connectionMetadata';
import { createLongFormPrismDID } from '@/actions';

export const ConnectToCA: React.FC = () => {
  const app = useMountedApp();
  const agent = app.agent.instance;
  const defaultSeed = app.defaultSeed;
  ...
  // 🆕 NEW: User name input state (REQUIRED for PRISM DID creation)
  const [userName, setUserName] = useState<string>('');

  // 🆕 NEW: Name validation error
  const [nameError, setNameError] = useState<string | null>(null);

  // 🆕 NEW: PRISM DID creation state
  ...
  const handleConnectToCA = async () => {
    ...
    // 🆕 REQUIRED: Validate name field (used as PRISM DID alias)
    if (!userName.trim()) {
      setNameError('Name is required to create your PRISM DID');
      console.log('❌ [CA CONNECT] Name is required but was empty');
      return;
    }
    setNameError(null);
    ...
    try {
      ...
      setError(null);
      setSuccess(false);
      setCreatedPrismDID(null);  // Reset PRISM DID state
      ...
      // ============================================
      // 🆕 STEP 1: Create PRISM DID with user's name as alias
      // This DID will represent the user's identity for CA interactions
      // ============================================
      console.log('🔑 [CA CONNECT] Creating PRISM DID with alias:', userName.trim());
      ...
      try {
        ...
        console.log('✅ [CA CONNECT] PRISM DID created:', newPrismDID.substring(0, 60) + '...');
        console.log('  → Alias:', userName.trim());
      } catch (prismError: any) {
        console.error('❌ [CA CONNECT] Failed to create PRISM DID:', prismError);
      ...
      // ============================================
      // 🆕 STEP 2: Proceed with CA connection
      // ============================================
      ...
      if (invitationData.type === 'https://didcomm.org/out-of-band/2.0/invitation') {
        ...
        try {
          ...
          // 🔧 FIX #8 (Dec 3, 2025): Replace 2-second timeout with retry logic to fix race condition
          console.log('🔧 [CA CONNECT] Waiting for connection to be created (with retry)...');

          // Retry loop to find the connection (up to 10 seconds total)
          let newConnection = null;
          let retries = 0;
          const maxRetries = 10;  // 10 attempts × 1 second = up to 10 seconds
          ...
          while (!newConnection && retries < maxRetries) {
            ...
            console.log(`🔍 [CA CONNECT] Connection search attempt ${retries}/${maxRetries}${newConnection ? ' - FOUND!' : ''}`);
          }
          ...
          if (newConnection) {
            ...
            // 🆕 Store PRISM DID association in connection metadata
            if (newPrismDID) {
              saveConnectionMetadata(newConnection.host.toString(), {
                walletType: 'local',
                prismDid: newPrismDID
              });
              console.log('✅ [CA CONNECT] PRISM DID associated with connection');
              console.log('  → Host DID:', newConnection.host.toString().substring(0, 40) + '...');
              console.log('  → PRISM DID:', newPrismDID.substring(0, 40) + '...');
            }
          } else {
            console.error('❌ [CA CONNECT] Failed to find connection after', maxRetries, 'attempts');
            console.error('❌ [CA CONNECT] PRISM DID will NOT be associated - credential acceptance will use SDK fallback');
            console.log('ℹ️ [CA CONNECT] Connection may still be established asynchronously via message handler');
          }
    ...
    try {
      ...
      if (caCredential) {
        ...
        try {
          ...
          // 🔧 FIX: Check if credential already exists before storing (prevent duplicates)
          const existingCreds = await agent.pluto.getAllCredentials();
          const credentialId = parsedCredential.id;

          const alreadyExists = existingCreds.some((cred) => {
            const existingId = cred.id;
            return existingId === credentialId;
          });

          if (alreadyExists) {
            console.log('ℹ️ [CA CONNECT] Credential already stored by SDK, skipping manual storage');
            console.log('   Credential ID:', credentialId);
          } else {
            // Store the parsed credential (now has isStorable() method)
            console.log('📝 [CA CONNECT] Storing CA credential manually...');
      ...
      // 🔧 FIX #8.1 (Dec 3, 2025): Pass createdPrismDID to modal path for proper association
  ...
  // 🔧 FIX #8.1 (Dec 3, 2025): Added prismDID parameter to fix modal path PRISM DID association
      ...
      if (invitationData.type === 'https://didcomm.org/out-of-band/2.0/invitation') {
        ...
        try {
          ...
          try {
            ...
            // Log all DID strings for debugging
            allOurDIDs.forEach((did, index) => {
              console.log(`🔍 [MEDIATOR REGISTER] DID ${index + 1}/${allOurDIDs.length}:`, did.toString());
            });

            if (allOurDIDs.length > 0) {
              console.log('📤 [MEDIATOR REGISTER] Calling updateKeyListWithDIDs()...');

              // Check if mediator is available
              const mediator = agent!.mediationHandler.mediator;
              if (!mediator) {
                console.error('❌ [MEDIATOR REGISTER] NO MEDIATOR CONFIGURED!');
                throw new Error('No mediator available for DID registration');
              }

              console.log('🔍 [MEDIATOR REGISTER] Mediator DID:', mediator.mediatorDID.toString());
              console.log('🔍 [MEDIATOR REGISTER] Our host DID:', mediator.hostDID.toString());
              ...
              console.log('✅ [MEDIATOR REGISTER] updateKeyListWithDIDs() completed successfully');

              // Give mediator time to process
              await new Promise(resolve => setTimeout(resolve, 2000));
              console.log('✅ [MEDIATOR REGISTER] Waited 2 seconds for mediator processing');
            }
          } catch (registerError: any) {
            console.error('❌ [MEDIATOR REGISTER] FAILED:', registerError);
            console.error('❌ [MEDIATOR REGISTER] Error details:', {
          ...
          // 🔧 FIX #8.1 (Dec 3, 2025): Associate PRISM DID with connection (modal path)
          if (prismDID) {
            console.log('🔧 [CA CONNECT - MODAL PATH] Looking for new connection to associate PRISM DID...');
            ...
            while (!newConnection && retries < maxRetries) {
              ...
              console.log(`🔍 [CA CONNECT - MODAL PATH] Connection search attempt ${retries}/${maxRetries}${newConnection ? ' - FOUND!' : ''}`);
            }
            ...
            if (newConnection) {
              ...
              console.log('✅ [CA CONNECT - MODAL PATH] Connection name updated');

              // Associate PRISM DID with connection
              saveConnectionMetadata(newConnection.host.toString(), {
                walletType: 'local',
                prismDid: prismDID
              });
              console.log('✅ [CA CONNECT - MODAL PATH] PRISM DID associated with connection');
              console.log('  → Host DID:', newConnection.host.toString().substring(0, 50) + '...');
              console.log('  → PRISM DID:', prismDID.substring(0, 50) + '...');
            } else {
              console.error('❌ [CA CONNECT - MODAL PATH] Connection not found after', maxRetries, 'attempts');
              console.error('  → PRISM DID NOT associated - credential binding may fail!');
            }
          }
  ...
  return (
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 mb-6">
      ...
      {/* 🆕 User Name Input (REQUIRED for PRISM DID creation) */}
      <div className="mb-4">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (nameError) setNameError(null); // Clear error on input
          }}
      ...
      <div className="flex justify-end">
        <button
          ...
          disabled={connecting || creatingPrismDID || !agent || success || !userName.trim()}
          className={`px-6 py-3 rounded-lg font-semibold transition-all duration-200 ${
            connecting || creatingPrismDID || !agent || success || !userName.trim()
              ? success
